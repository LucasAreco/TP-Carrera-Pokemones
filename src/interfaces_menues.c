...
void mostrar_obstaculos_oponente(TP *tp, enum TP_JUGADOR jugador,
				 int dificultad,
				 unsigned int cantidad_obstaculos)
{
...
void mostrar_interfaz_info_oponente(void *contexto)
{
	...
	mostrar_obstaculos_oponente(estado->juego, JUGADOR_2,
				    estado->dificultad, cantidad_oponentes);
...
void mostrar_resultado_carrera(void *contexto)
{
	...
	int diferencia_tiempos_signed =
		abs((int)tiempo_usuario - (int)tiempo_oponente);
	...
	unsigned puntaje_final =
		100 - (100 * diferencia_tiempos) / suma_tiempos;
