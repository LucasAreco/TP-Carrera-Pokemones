 ...
 */
void completar_substring(char *substring, const char *string,
			 size_t pos_separador, size_t pos_anterior_separador)
{
 ...
 */
char *inicializar_substring(const char *string, size_t posicion_actual,
			    size_t pos_anterior_separador)
{
	...
	completar_substring(substring, string, posicion_actual,
			    pos_anterior_separador);
	return substring;
}
 ...
 */
size_t obtener_cantidad_separadores(const char *string, char separador,
 ...
 */
void dividir_cadena(char **string_a_separar, const char *string, char separador,
		    size_t longitud_string)
{
	...
	for (size_t i = 0; i <= longitud_string; i++) {
		if (string[i] == separador || string[i] == FINAL_STR) {
			char *substring = inicializar_substring(
				string, i, pos_anterior_separador);
			if (substring == NULL) {
...
char **split(const char *string, char separador)
{
	...
	size_t cantidad_separadores = obtener_cantidad_separadores(
		string, separador, longitud_string);

	char **string_separado =
		malloc(sizeof(char *) * (cantidad_separadores + 1));
